...
if($rand_dom)
{
	...
	if(pid)
	{
		...
		$rand_dom.html(typeof d == 'string' ? d : d.toISOString());
	}
}
var countdown_trans = {
	label_dd: section.settings.trans_label_dd,
	label_hh: section.settings.trans_label_hh,
	label_mm: section.settings.trans_label_mm,
	label_ss:  section.settings.trans_label_ss,
	css_class: 'countdown-alt-2'

};
jQuery.fn.countDown &&  $_this.find('.count_time').countDown(countdown_trans);
$_this.find('.count_time').on('time.elapsed', function () {
		 $_this.find('.lh-countdown-product').remove()
});
 };						cb.bind($('section[data-section-id="1vPwJ4Wd3u"] include[data-name="countdown"][data-id="2"]').get(0))(jQuery);;var cb  = function($) {
...
var current_variant = null;
	...
	_this.closest('.lh-product-single').find('.lh-close-modal').click(function() {
	    $(this).closest('.lh-modal').fadeOut(100);
	})
	...
	function ProductOptionsInit() {
	    ...
	    function selectCallback(variant, selector) {
          current_variant = variant;
					...
					//var eco = $this.find('.lh-product-economisez');
					...
					if(variant){
						...
						sold_out.css({"display":"block"}).find('span').attr("data-lhi","trans_unavai").html(section.settings.trans_unavai);
	        ...
	        if (isSale) {
							...
							//eco.find('span.lh-eco-percent').text(Math.round((current_variant.compare_at_price / current_variant.price * 100 - 100 ) * 10) / 10 + "%");
							//eco.find('span.lh-eco-price').text(window.HUB.formatMoney(current_variant.compare_at_price - current_variant.price,money_format))
	    ...
	    $this.find('.lh-swatch-select').on('click', function() {
	        var _this = $(this);
		        ...
		        $this.find('select#product-select-' + id + '-option-' + index).val(_this.data('value')).trigger('change');
						//checkExistsVariant(_this.data('optionIndex'),_this.data('value'));
					}
	    });
	...
	//checkExistsVariant(first_variant_block.data('optionIndex'),first_variant_block.data('value'))
	_this.find('.lh-swatch-select').addClass('lh-exists');
	function checkExistsVariant(index,value){
		_this.find('.lh-swatch-variations .lh-swatch-select.lh-exists').removeClass('lh-exists');
		_this.find('.lh-swatch-variations[data-option-index="'+(index)+'"] .lh-swatch-select').addClass('lh-exists');
		for (var i = 1; i <= 3; i++) {
			if(i == (index + 1)) continue;
			if($product_json && $product_json['options'][i-1]){

				$product_json.variants.forEach(function(variant){

					if(variant['option' + (index + 1)] == value){

						var current = _this.find('.lh-swatch-variations[data-option-index="'+(i - 1)+'"]');
						if(variant.available){
							current.find('.lh-swatch-select[data-value="'+variant['option' + i]+'"]').addClass('lh-exists');
						}
					}
				})
			}
 ...
 };						cb.bind($('section[data-section-id="1vPwJ4Wd3u"] include[data-name="product-form"][data-id="3"]').get(0))(jQuery);;var cb  = function($) {
  var $this = jQuery(this);
  $this.find('.lh-nav-tabs li').on('click','a',function(event){
     event.preventDefault();
     var $_this = jQuery(this),
         tab_id = $_this.attr('data-tab');
    if(!$_this.hasClass('active')){
      $this.find('.tab-link.active').removeClass('active');
      $_this.toggleClass('active');
      $this.find(".lh-tab-content.active").removeClass('active');
      $this.find("#"+tab_id).addClass('active');
   }
...
if ($review && (window.LAYOUTHUB_LIVE    || window.location.protocol==='blob:')) {
 ...
 };						cb.bind($('section[data-section-id="1vPwJ4Wd3u"] include[data-name="tabs"][data-id="5"]').get(0))(jQuery);;var cb  = function($) {
 ...
 };						cb.bind($('section[data-section-id="1vPwJ4Wd3u"]').get(0))(jQuery);};					cb.bind($('section[data-section-id="1vPwJ4Wd3u"]').get(0))({						url: "https://library.layouthub.com/HUB/files/Tm92LS0yMDE5/NTc0ODY0ODc5/First_release/pages/Gent-Detail/product/",						settings: {"trans_label_dd":"days","trans_label_hh":"hours","trans_label_mm":"minutes","trans_label_ss":"seconds","trans_unavai":"Unavailable","trans_outstock":"Out of stock"}					}, jQuery);})(jQuery);;(function($){var ajax = (p, c, e) => { $.ajax({url: window.location.href, method: "POST",							data: $.extend(true, p !== undefined && typeof p == "object" ? p : {}, {								action: "layouthub_section_ajax", section_id: "o1cjQLKh0G"							}), success: c, error: e});						}, cb  = function(section, $) {/**
 * Owl Carousel v2.3.4
...
window.HUB.formatMoney = window.HUB.formatMoney ||  function(t, r) {
    ...
    return i.replace(a, o)
		...
		function adjustDots(){
			var max_height = 0,
					stage_slide = $this.find('.lh-product-related .owl-stage-outer');
			$this.find('.lh-product-info').each(function(){
				var this_height = $(this).outerHeight();
				max_height = (this_height > max_height) ? this_height : max_height;
			});
			stage_slide.css('padding-bottom', max_height - 67 + 35);
		}
	 	...
	 	function initSlide(){
			$this.find('.owl-carousel').owlCarousel({
		        ...
		        margin:0,
						nav: true,
		        dots: true,
						navText: [`<svg version="1.1" width="16" height="16" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
		        ...
		        responsive:{
			        0:{
			            items: section.settings.show_2 == 'yes'?2:1
			        },
			        480:{
			            items:2
			        },
...
$(window).resize(function(){
 ...
 };						cb.bind($('section[data-section-id="o1cjQLKh0G"] include[data-name="product-grid"][data-id="1"]').get(0))(jQuery);;var cb  = function($) {
  if(document.querySelector('.page-container') && window.getComputedStyle(document.querySelector('.page-container')).transform.indexOf('matrix') != -1){
    document.querySelector('.page-container').style.webkitTransform='none';
    document.querySelector('.page-container').style.transform='none';
  }
 };						cb.bind($('section[data-section-id="o1cjQLKh0G"]').get(0))(jQuery);};					cb.bind($('section[data-section-id="o1cjQLKh0G"]').get(0))({						url: "https://library.layouthub.com/HUB/files/Tm92LS0yMDE5/NTc0ODY0ODc5/First_release/pages/Gent-Detail/o-product/",						settings: {"show_2":"no"}					}, jQuery);})(jQuery);;console.log('This page layout has been built by https://www.layouthub.com');